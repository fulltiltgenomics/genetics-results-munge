...
for line in f:
    ...
    gene_end = s[4]
    gene_strand = s[6]
    fields = [f.strip() for f in s[8].split(";")]
    gene_id = (
        [f for f in fields if f.startswith("gene_id ")][0]
        .split(" ")[1]
        .replace('"', "")
    )
    gene_name = (
        [f for f in fields if f.startswith("gene_name ")][0]
        .split(" ")[1]
        .replace('"', "")
    )
    gene_type = (
        [f for f in fields if f.startswith("gene_type ")][0]
        .split(" ")[1]
        .replace('"', "")
    )
    outfile_handle.write(
